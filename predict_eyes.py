...
model = joblib.load('drowsiness_model.joblib')

# Load the facial landmark predictor and dlib face detector
detector = dlib.get_frontal_face_detector()
...
while True:
    ...
    for rect in rects:
        ...
        prediction = model.predict(np.array([[avg_ear]]))

        # Display the result
        label = "Closed" if prediction[0] == 0 else "Open"
        ...
        # Draw the eyes
        draw_eye(frame, left_eye, (255, 0, 0), is_left=True)  # Blue for left eye
        draw_eye(frame, right_eye, (255, 0, 0), is_left=False)  # Blue for right eye

        # Show the EAR and prediction on the frame
        cv2.putText(frame, f"EAR: {avg_ear:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
        cv2.putText(frame, label, (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)

        # Alert for drowsiness (optional)
        if prediction[0] == 0:  # Drowsy (eyes closed)
            cv2.putText(frame, "Drowsiness Detected!", (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
